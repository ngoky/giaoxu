...
export const AppHeader = () => {
    ...
    const contextRoutes:any[] = location.pathname.split('/') || []
    const { t } = useTranslation()
    const mapToName: any[] = t('menus.site-map-text', { returnObjects: true }) || []
    let realObjects:any = []
    if (contextRoutes.length > 0) {
        realObjects = contextRoutes.map((x, index) => {
            const array = [...contextRoutes.filter((x) => x)].splice(0, index)
            const display = mapToName.find((map: { to: any }) => map.to === x)
