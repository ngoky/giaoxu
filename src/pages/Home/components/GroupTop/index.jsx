...
const FirstPostRow = ({ post, theme }) => {
    return (
        <Typography title={post.title} component="a" href={`/news/${post.id}`}>
            <ListItemButton
                style={{ maxHeight: '60px', marginLeft: 10, padding: 0 }}
            >
...
const TopNews = ({ post, theme }) => {
    return (
        <Typography title={post.title} component="a" href={`news/${post.id}`}>
            <ListItemButton
                style={{ maxHeight: '60px', marginLeft: 0, padding: 0 }}
            >
...
const GroupTop = () => {
    ...
    const topNews = useSelector((state) => state.posts?.typeTopNews || {})
    // const { posts = [] } = topNews

    return <ChildView id="ChildView" data={topNews} />
}
